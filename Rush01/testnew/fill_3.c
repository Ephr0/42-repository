/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   fill_3.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: npaturzo <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/28 10:22:01 by npaturzo          #+#    #+#             */
/*   Updated: 2023/05/28 17:02:40 by bchuluun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

void	write_out(int output[][4], int x, int write[]);
void	opp1(int column, int output[][4], int count[2][4]);

int	fill_3(int column, int opposite, int output[][4], int count[2][4])
{
	int	write[4];

	if (opposite == 1)
	{
		opp1(column, output, count);
		return (0);
	}
	if (opposite == 2)
	{
		count[1][column] = 3;
		if (count[0][column] == 0)
			opp1(column, output, count);
		if (count[0][column] == 1)
			opp2(column, output, count);
		if (count[0][column] == 2)
		{
			write[0] = 2;
			write[1] = 3;
			write[2] = 4;
			write[3] = 1;
			write_out(output, column, write);
		}
	}
	if (opposite > 2)
		return (1);
}

void	opp1(int column, int output[][4], int count[2][4])
{
	int	write[4];

	count[1][column] = 2;
	if (count[0][column] == 0)
	{
		write[0] = 2;
		write[1] = 1;
		write[2] = 3;
		write[3] = 4;
		write_out(output, column, write);
	}
	if (count[0][column] == 1)
	{
		write[0] = 2;
		write[1] = 3;
		write[2] = 1;
		write[3] = 4;
		write_out(output, column, write);
	}
}

void	opp2(int column, int output[][4], int count[2][4])
{
	int	write[4];

	if (count[0][column] == 0)
	{
		write[0] = 1;
		write[1] = 2;
		write[2] = 4;
		write[3] = 3;
		write_out(output, column, write);
	}
	if (count[0][column] == 1)
	{
		write[0] = 1;
		write[1] = 3;
		write[2] = 4;
		write[3] = 2;
		write_out(output, column, write);
	}
}
